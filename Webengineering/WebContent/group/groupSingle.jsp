...
<jsp:useBean id="singleGroup" class="main.java.beans.SingleGroupBean"
	scope="session"></jsp:useBean>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<title>MyWG - <c:out value="${singleGroup.getName()}"></c:out></title>
			...
			<div class="backgroundMid">
				<div class="container">
					<div class="overviewHeader">
						<h1>
							<u><c:out value="${singleGroup.getName()}"></c:out></u>
						</h1>
						<p>
							Beschreibung:
					...
					<div class="widget justify-content-center">
						...
						<form action="../payment/MonthlyPayment">
							<input type="hidden" name="monthlyPaymentID"
								id="monthlyPaymentID" />
							<c:forEach items="${singleGroup.getMonthlyPayments() }"
								var="monthlyPayment">
								<p class="singlePaymentFromOverview">
									Erstellungs Datum:
									<c:out value="${monthlyPayment.formatDateCreated() }" />
									<br> Zu Zahlen bis:
									<c:out value="${monthlyPayment.formatDateUntil() }" />
									<br> <br>

									<button class="btn btn-success" type="submit"
										onclick="setMonthlyPaymentID(${monthlyPayment.getMonthlyPaymentID()})">Zur
										Zahlung</button>
									<br> <br>
								</p>
							</c:forEach>
					...
					<form name="createPayment" action="createPayment" method="POST"
						...
						<div class="modal-dialog">
							<input type="hidden" name="groupID"
								value="${singleGroup.getGroupID()}" />
							<div class="modal-content">
								<!-- Modal Header -->
								<div class="modal-header">
									<h4 class="modal-title mx-auto">Neue Zahlung Erstellen</h4>
									<button type="button" class="close" data-dismiss="modal"
										onclick="clearPaymentModal()">&times;</button>
								</div>
								<!-- Modal body -->
								<div class="modal-body">
									<div class="form-group">
										<label for="paymentValue">Betrag:</label> <input type="text"
											class="form-control" id="paymentValue" name="paymentValue"
											onkeyup="checkPaymentModal()">
									</div>
									<div class="form-group">
										<label for="paymentdescription">Beschreibung:</label> <input
											type="text" class="form-control" id="paymentdescription"
									...
									<div class="row mx-1 pb-3 text-center">
										...
										<div class="col-md-* mr-1" id="leftModal">
											<input type="text" class="form-control autocomplete" id="P1"
												name="P1" placeholder="Name">
										</div>
										<!-- RIGHT -->
										<div class="col-md-*" id="rightModal">
											<input type="text" class="form-control" id="P1P" name="P1P"
												placeholder="Beteiligung in %" onkeyup="checkPaymentModal()">
										</div>
									</div>
									<a class="btn btn-secondary mb-4" style="color: #fff"
										onclick="addUser()">Teilnehmer Hinzufügen</a>
									<!-- Modal footer -->
									<div class="modal-footer">
										<button type="button" class="btn btn-danger"
											data-dismiss="modal" onclick="clearPaymentModal()">Abbrechen</button>
										<button id="submitPayment" type="submit"
											class="btn btn-primary">Zahlung erstellen</button>
									</div>
								</div>
							</div>
						</div>
