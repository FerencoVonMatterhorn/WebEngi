...
<jsp:useBean id="monthlyPayment" class="main.java.beans.MonthlyPaymentBean" scope="session"></jsp:useBean>
				...
				<div class="container">
					...
					<div class="row justify-content-center">
						<h1 class="pt-5 text-center">
							...
							.
							<c:out value="${monthlyPayment.getMonthlyPayment().getDateCreated().getYear()}" />
						</h1>
					</div>
					<!--  row Überschrift END  -->
					<!--  row Table  -->
					<form action="../payment/SinglePayment">
						<div class="row col-10 mx-auto mt-5 mb-5">
							...
							<table class="table table-striped bg-dark text-white">
								...
								<tbody>
									<c:forEach items="${monthlyPayment.getPayments()}" var="payment" varStatus="loop">
										<tr>
											<td><c:out value="${payment.getPaymentDescription()}" /></td>
											<td><button class="paymentBtn btn btn-success" type="submit" onclick="setPaymentID(${payment.getPaymentID()})">Zur
													Zahlung</button></td>
											<td><c:out value="${monthlyPayment.getSingleAmount(monthlyPayment.getPaymentToUser().get(loop.index), payment)}" /></td>
										</tr>
									</c:forEach>
